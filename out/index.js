...
class QuickDB {
    ...
    async addSubtract(key, value, sub = false) {
        if (typeof key != "string") {
            throw new error_1.CustomError(`Đối số đầu tiên (key) phải là chuỗi, nhưng nhận được "${typeof key}"`, error_1.ErrorKind.InvalidType);
        }
        if (value == null) {
            throw new error_1.CustomError("Thiếu đối số thứ hai (value)", error_1.ErrorKind.MissingValue);
        }
        ...
        if (typeof currentNumber != "number") {
            try {
                ...
                throw new error_1.CustomError(`Giá trị hiện tại với key: (${key}) không phải là số và không thể ép sang số`, error_1.ErrorKind.InvalidType);
            }
        }
        if (typeof value != "number") {
            try {
                ...
                throw new error_1.CustomError(`Giá trị để cộng/trừ với key: (${key}) không phải là số và không thể ép sang số`, error_1.ErrorKind.InvalidType);
            }
        }
    ...
    static registerSingleton(name, options = {}) {
        if (typeof name != "string") {
            throw new error_1.CustomError(`Đối số đầu tiên (name) phải là chuỗi, nhưng nhận được "${typeof name}"`, error_1.ErrorKind.InvalidType);
        }
    ...
    static getSingleton(name) {
        ...
        if (!instance) {
            throw new error_1.CustomError(`Không tìm thấy instance singleton với tên "${name}"`, error_1.ErrorKind.InstanceNotFound);
        }
        return instance;
    }
    async init() {
    ...
    async update(key, object) {
        ...
        if (typeof object != "object" || object == null) {
            throw new error_1.CustomError(`Đối số thứ hai (object) phải là object, nhưng nhận được "${typeof object}"`, error_1.ErrorKind.InvalidType);
        }
        const data = (await this.get(key)) ?? {};
        if (typeof data != "object" || Array.isArray(data)) {
            throw new error_1.CustomError(`Dữ liệu hiện tại không phải là object, update chỉ hoạt động với object`, error_1.ErrorKind.InvalidType);
        }
    ...
    async startsWith(query) {
        if (typeof query != "string") {
            throw new error_1.CustomError(`Đối số đầu tiên (query) phải là chuỗi, nhưng nhận được "${typeof query}"`, error_1.ErrorKind.InvalidType);
        }
    ...
    async table(table) {
        if (typeof table != "string") {
            throw new error_1.CustomError(`Đối số đầu tiên (table) phải là chuỗi, nhưng nhận được "${typeof table}"`, error_1.ErrorKind.InvalidType);
        }
