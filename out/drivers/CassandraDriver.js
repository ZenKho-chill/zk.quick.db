...
class CassandraDriver {
  ...
  checkConnection() {
    if (this._client === null) {
      throw new Error("Không có kết nối đến cơ sở dữ liệu Cassandra");
    }
  }
  async prepare(table) {
    this.checkConnection();
    await this._client.execute("CREATE KEYSPACE IF NOT EXISTS zkquickdb WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': '1' }");
    await this._client.execute("USE zkquickdb");
