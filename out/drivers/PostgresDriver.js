...
class PostgresDriver {
  ...
  checkConnection() {
    if (!this.conn) {
      throw new Error(`PostgresDriver đang không kết nối tới database!`);
    }
  }
  async prepare(table) {
    this.checkConnection();
    await this.conn.query(`CREATE TABLE IF NOT EXISTS ${table} (id VARCHAR(25), value TEXT)`);
  }
  ...
  async getStartsWith(table, query) {
    this.checkConnection();
    const queryResult = await this.conn.query(`SELECT * FROM ${table} WHERE ID LIKE $1`, [`${query}%`]);
    return queryResult.rows.map((row) => ({
      id: row.id,
      value: JSON.parse(row.value),
    }));
